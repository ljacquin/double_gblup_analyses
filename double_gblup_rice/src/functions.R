...
krmm_ <- function(Y, X = rep(1, length(Y)), Z = diag(1, length(Y)), Matrix_covariates,
                  method = "RKHS", kernel = "Gaussian", center_covariates = T,
                  scale_covariates = F, rate_decay_kernel = 0.1, degree_poly = 2,
                  scale_poly = 1, offset_poly = 1, degree_anova = 3, init_sigma2K = 2,
                  init_sigma2E = 3, convergence_precision = 1e-08, nb_iter = 1000,
                  use_prior_comp_estim = T, mu, display = F) {
  ...
  if (sum(is.nan(Matrix_covariates)) > 0) {
    warning("Scaling produces NaN, the covariates matrix may contain columns with zero\n
    ...
    if (identical(method, "RKHS")) {
      if (identical(kernel, "Gaussian")) {
        ...
        kernel_function <- laplacedot(sigma = (1 / p) *
          rate_decay_kernel)
      ...
      if (identical(method, "GBLUP") || identical(
        method,
        "RR-BLUP"
      )) {
    ...
    K_inv <- ginv(K / max(K))
    if (use_prior_comp_estim) {
      beta_hat <- as.vector(mu)
      sigma2K_hat <- as.vector(init_sigma2K)
      sigma2E_hat <- as.vector(init_sigma2E)
    } else {
      MM_components_solved <- em_reml_mm(
        K_inv, Y, X, Z, init_sigma2K,
    ...
    var_y_div_sig2_alpha <- crossprod(t(Z), tcrossprod(
      K,
      Z
    ...
    if (identical(method, "RKHS") || identical(method, "GBLUP")) {
      return(list(
        X = X, Z = Z, Matrix_covariates = Matrix_covariates,
        beta_hat = beta_hat, sigma2K_hat = sigma2K_hat,
        sigma2E_hat = sigma2E_hat, vect_alpha = vect_alpha,
        method = method, kernel = kernel, rate_decay_kernel = rate_decay_kernel,
        degree_anova = degree_anova, degree_poly = degree_poly,
        scale_poly = scale_poly, offset_poly = offset_poly,
        covariates_center = covariates_center, covariates_scale = covariates_scale
      ))
    } else if (identical(method, "RR-BLUP")) {
      gamma_hat <- crossprod(Matrix_covariates, vect_alpha)
      return(list(
        X = X, Z = Z, Matrix_covariates = Matrix_covariates,
        beta_hat = beta_hat, gamma_hat = gamma_hat, sigma2K_hat = sigma2K_hat,
        sigma2E_hat = sigma2E_hat, vect_alpha = vect_alpha,
        method = method, covariates_center = covariates_center,
